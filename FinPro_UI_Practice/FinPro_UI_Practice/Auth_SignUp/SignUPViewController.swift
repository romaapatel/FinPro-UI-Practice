...
class SignUPViewController: UIViewController {
    ...
    @IBAction func onClickOfBtnCreateAccount(_ sender: Any) {
        let isValidEmail = tvEmail.validate(validationType: .email)
        let isValidPassword = passwordTextField.validate(validationType: .password)
        let isValidName = tvName.validate(validationType: .requiredField)
        
        if (isValidName && isValidEmail && isValidPassword) {
            let user = User(email: tvEmail.text ?? "jb", password: passwordTextField.text ?? "jkb")
            sendPostRequest(requestType: .post, apiType: .signUp, body: user, apiResponse: self)
        }
    }
